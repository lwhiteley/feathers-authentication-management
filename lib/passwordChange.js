...
module.exports = function passwordChange(options, identifyUser, oldPassword, password, passwordFieldName) {
  ...
  var errMsg = 'Current ' + passwordField + ' is incorrect.';
  ...
  return Promise.resolve().then(function () {
    ...
    return Promise.all([user1, hashPassword(options.app, password), comparePasswords(oldPassword, user1[passwordField], function () {
      return new errors.BadRequest(errMsg, { errors: { oldPassword: errMsg } });
    })]);
  }).then(function (_ref) {
    ...
    return (// value from comparePassword is not needed
      patchUser(user1, _defineProperty({}, passwordField, hashedPassword))
    );
  }).then(function (user1) {
    return notifier(options.notifier, action, user1, { passwordField: passwordField });
  }).then(function (user1) {
